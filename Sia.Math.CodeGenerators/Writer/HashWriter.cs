...
public class HashWriter(VectorType type, bool wide): IMathSourceWriter
{
    ...
    private static uint s_NextPrime = 0;

    private static uint NextPrime() => s_Primes[s_NextPrime++ & 255];
    ...
    public Action<IndentedTextWriter> MathSourceWriter => source =>
    {
        ...
        {
            source.WriteLine("/// <summary>Returns a uint hash code of a {0} {1}.</summary>", type.TypeName, matrixOrVector);
        }
        ...
        source.Indent++;
        {
            ...
            if (type.BaseType == "bool")
            {
                for (var column = 0; column < type.Columns; column++)
                {
                    ...
                    {
                        source.WriteLine(" + ");
                        source.Write(new string(' ', align.Length));
                    }

                    var columnName = type.Columns > 1 ? "v.c" + column : "v";
                    source.Write($"select({FormatPrimeUIntVector(type.Rows)}, {FormatPrimeUIntVector(type.Rows)}, {columnName})");
                }
            ...
            else
            {
                for (var column = 0; column < type.Columns; column++)
                {
                    ...
                    source.Write("{0} * {1}", columnName, FormatPrimeUIntVector(type.Rows));
                }

                source.WriteLine(") + 0x{0:X}u;", NextPrime());
            }
        }
        source.Indent--;
    };
    ...
    private static StringBuilder FormatPrimeUIntVector(int n)
    {
        ...
        builder.AppendFormat("uint{0}(", n);
        for (var row = 0; row < n; row++)
        {
            if (row != 0) builder.Append(", ");
            builder.AppendFormat("0x{0:X}u", NextPrime());
        }
        builder.Append(")");
